...
Третье отличие — кадрирование баннера. Размеры `.catalog-hero` те же, что на остальных разделах (460 px на десктопе), но у картинки задан `object-position: center top` вместо `center 40%`: композиция печи должна стоять ниже в кадре. Скрипт баннер не подменяет — раздел один, адреса картинок с версией `?v=` заданы прямо в разметке.
