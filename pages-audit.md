...
**Ширина футера привязана к длине подписей ссылок.** Обновлено 2026-07-29. Колонки `.footer__top` заданы как `repeat(4, max-content)`, то есть их ширина определяется текстом ссылок. Расстояния между колонками подобраны так, чтобы футер совпадал по ширине с шапкой: верхние границы `column-gap` — 171 px на экранах шире 1500 и 121 px в полосе 1101–1500 (`style.css`, правила `.footer__top` и `.footer__bottom`). **Если менять тексты ссылок в футере, эти границы нужно пересчитать**, иначе футер разъедется с шапкой. Формула: `граница = (ширина шапки − суммарная ширина колонок) / 3`.
